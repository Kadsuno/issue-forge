<x-app-layout>
            ...
            <div class="flex items-start md:items-center gap-3 md:gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-slate-400">Update project details and settings</p>
                    </div>
                </div>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="card p-6 sm:p-8 animate-fade-in-up">
                ...
                <form action="{{ route('projects.update', $project->slug) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $project->name) }}"
                            required class="input w-full" placeholder="Enter project name...">
                        @error('name')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="default_assignee_id" name="default_assignee_id" class="input w-full">
                                ...
                                @foreach (\App\Models\User::orderBy('name')->get() as $user)
                                    <option value="{{ $user->id }}" @selected(old('default_assignee_id', $project->default_assignee_id) == $user->id)>
                        ...
                        <div>
                            ...
                            @php($vis = old('visibility', $project->visibility ?? 'team'))
                        ...
                        <div>
                            ...
                            <input type="text" id="ticket_prefix" name="ticket_prefix" maxlength="10"
                                value="{{ old('ticket_prefix', $project->ticket_prefix) }}" class="input w-full"
                                placeholder="e.g. WEB">
                            @error('ticket_prefix')
                                <p class="mt-1 text-sm text-danger-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="text" id="color" name="color"
                                value="{{ old('color', $project->color ?? '#1e40af') }}" class="input w-full"
                                placeholder="#1e40af">
                            @error('color')
                                <p class="mt-1 text-sm text-danger-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            @php($p = old('priority', $project->priority ?? 'medium'))
                    ...
                    <div
                        ...
                        <div class="flex flex-col sm:flex-row gap-3 order-1 sm:order-2 w-full sm:w-auto">
                            <!-- Delete Project -->
                            <button type="button" onclick="confirmDelete()" class="btn-danger w-full sm:w-auto">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <!-- Hidden Delete Form -->
                <form id="deleteForm" action="{{ route('projects.destroy', $project->slug) }}" method="POST"
                    class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
            ...
            <div class="mt-8 card p-6 animate-fade-in-up" style="animation-delay: 0.2s;">
                <div class="flex items-start space-x-3">
                    ...
                    <div class="flex-1">
                        <h3 class="text-sm font-semibold text-white mb-3">Project Information</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                            <div>
                                <span class="text-slate-500">Created:</span>
                                <span class="text-slate-300 ml-2">{{ $project->created_at->format('M j, Y') }}</span>
                            </div>
                            <div>
                                <span class="text-slate-500">Last updated:</span>
                                <span class="text-slate-300 ml-2">{{ $project->updated_at->diffForHumans() }}</span>
                            </div>
                            <div>
                                <span class="text-slate-500">Tickets:</span>
                                <span class="text-slate-300 ml-2">{{ $project->tickets->count() }} total</span>
                            </div>
                            <div>
                                <span class="text-slate-500">Status:</span>
                                <span class="badge-{{ $project->is_active ? 'success' : 'warning' }} ml-2">
    ...
    <script>
        function confirmDelete() {
            if (confirm(
                    'Are you sure you want to delete this project? This action cannot be undone and will also delete all associated tickets.'
                )) {
